...
object GraphAPId {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 最终返回一个包含聚合消息的顶点集 VertexRDD[Msg],没有接收到消息的顶点不包含在返回的VertexRDD[Msg]中.
    ...
    // 计算度信息
    // 最一般的聚合任务就是计算顶点的度，即每个顶点相邻边的数量。在有向图中，经常需要知道顶点的入度、出度以及总共的度。
    // GraphOps 类包含一个操作集合用来计算每个顶点的度。例如，下面的例子计算最大的入度、出度和总度。
